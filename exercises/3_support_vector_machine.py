...
def plot_decision_function_helper(X, y, clf):
	...
	plot.scatter(X['Test 1'], X['Test 2'], c = np.where(dataframe['Final Test'].values == 1,'g','r'), edgecolors='black')
	ax = plot.gca()
	xlim = ax.get_xlim()
	ylim = ax.get_ylim()
	# print(xlim)
...
def plot_data(X, y, clf):
	...
	plot.figure()
	plot.title("Support Vector Machine: Final test based on Test 1 & 2")
	plot.xlabel("Test 1 score")
	plot.ylabel("Test 2 score")
	
	plot_decision_function_helper(X, y, clf)
	
	legend_passed = patch.Patch(label='Passed', color='green', edgecolor='black')
	legend_failed = patch.Patch(label='Failed', color='red', edgecolor='black')
	legend_boundary = patch.Patch(label='Decision Boundary', color='yellow', edgecolor='black')
	plot.legend(handles=[legend_passed, legend_failed, legend_boundary])
	plot.show()
...
plot_data(x_values_train, y_values_train, clf)


clf_predictions = clf.predict(x_values_test)
